            ...
            if (data.trim()) {
                try {
                    ...
                    if (!Array.isArray(loadedData)) {
                        ...
                        // Aggiunge automaticamente le proprietà mancanti per i dati esistenti
                        const processedData = loadedData.map((item, index) => {
                            const processedItem = { ...item };

                            // Aggiunge ID se mancante
                            if (!processedItem.id) {
                                processedItem.id = index + 1;
                            }

                            // Aggiunge createdAt se mancante
                            if (!processedItem.createdAt) {
                                const defaultDate = new Date('2025-06-21T10:00:00.000Z');
                                processedItem.createdAt = defaultDate.toISOString();
                            }

                            // Aggiunge updatedAt se mancante
                            if (!processedItem.updatedAt) {
                                processedItem.updatedAt = processedItem.createdAt;
                            }
